...
if (libErrors.length) fail('part library errors (run: node tools/check-parts.mjs):\n  ' + libErrors.join('\n  '));
const LIB_NAME = '06z_part_library.generated.js';
const sources = [...jsFiles.map((f) => ({ name: f, text: read(join(jsDir, f)) })),
  { name: LIB_NAME, text: `/* ==== PART LIBRARY (generated by build.mjs, do not edit) ==== */\nconst PART_LIBRARY = ${JSON.stringify(lib)};` }]
  .sort((a, b) => (a.name < b.name ? -1 : 1));
let js = sources.map((f) => `/* ---------- ${f.name} ---------- */\n${f.text}`).join('\n');
...
console.log(
  ...
  `(game.js ${(Buffer.byteLength(js) / 1024).toFixed(1)} KB, ${jsFiles.length} JS files, ${Object.keys(lib.parts).length} parts, ${Object.keys(lib.vehicles).length} vehicles; site ${(total / 1024).toFixed(0)} KB)`
);
